            ...
            {transactions.map((trx) => (
              <div key={trx.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-5 border border-slate-100 bg-slate-50 rounded-xl gap-4 hover:shadow-sm transition-shadow">
                ...
                <div className="flex flex-col items-end gap-2">
                  <span className="font-bold text-primary text-lg">Rp{trx.amount.toLocaleString("id-ID")}</span>
                  {trx.status === "success" && (
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {trx.packageId && (
                        <Link
                          ...
                          className="text-sm font-semibold text-white bg-primary px-4 py-1.5 rounded-lg hover:bg-primary/90 transition-colors"
                        >
                          Lanjut Bayar
                        </Link>
                      )}
